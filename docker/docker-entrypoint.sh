...
if [ -f data/previous.txt ]; then
  ...
  versions=$(wget --no-verbose --output-document - http://merlot.tools.ietf.org/dailydose/data/ | perl -ne 'if (/href=\"([0-9]+)/) { print $1, "\n"; }' | sort | tail -2)
  for version in $versions; do
    wget --recursive --no-parent --level=1 --no-clobber --no-host-directories --directory-prefix=data --cut-dirs=2 --no-verbose http://merlot.tools.ietf.org/dailydose/data/$version/
